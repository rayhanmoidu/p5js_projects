  ...
  var QuickSettings = {
    ...
    setSize: function (w, h) {
      ...
      this._content.style.height =
        h - this._titleBar.offsetHeight + "px";
      return this;
    },
    ...
    _drag: function (event) {
      ...
      this.setPosition(
        x + mouseX - this._startX,
        y + mouseY - this._startY
      );
      this._startX = mouseX;
      this._startY = mouseY;
      event.preventDefault();
    ...
    addImage: function (title, imageURL, callback) {
      ...
      this._controls[title] = {
        ...
        setValue: function (url) {
          ...
          if (callback) {
            streetViewImage.addEventListener(
              "load",
              function _onLoad() {
                streetViewImage.removeEventListener("load", _onLoad);
                callback(url);
              }
            );
          }
    ...
    _addNumber: function (
      ...
      var className =
        type === "range" ? "qs_range" : "qs_text_input qs_number";
    ...
    addProgressBar: function (title, max, value, valueDisplay) {
      var container = this._createContainer(),
        ...
        progressDiv = createElement(
          "div",
          null,
          "qs_progress",
          container
        ),
        valueDiv = createElement(
          "div",
          null,
          "qs_progress_value",
          progressDiv
    ...
    setProgressMax: function (title, max) {
      ...
      control.valueDiv.style.width =
        (control.value / control.max) * 100 + "%";

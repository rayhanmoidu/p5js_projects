class Hill {
    ...
    drawStartPositions(graphicsObject) {
        ...
        for (let i = 0; i < this.vertices.length - 1; i++) {
            graphicsObject.stroke(255, 255, 0);
            graphicsObject.strokeWeight(5)
            graphicsObject.line(this.vertices[i].getX(), this.vertices[i].getY(), this.vertices[i + 1].getX(), this.vertices[i + 1].getY())
        }

        for (let i = 0; i < this.startpos_piecewise.length - 1; i++) {
            graphicsObject.stroke(50, 230, 0);
            graphicsObject.strokeWeight(5)
        ...
        for (let i = 0; i < this.vertices.length - 1; i++) {
            if (endpos_x >= this.vertices[i].getX() && endpos_x <= this.vertices[i+1].getX()) {
                ...
                lala1 = startpos.add(diff.scalarmult(perc));
            }
        }

        endpos_x = this.endpos_max;
        let lala2;
        for (let i = 0; i < this.vertices.length - 1; i++) {
            if (endpos_x >= this.vertices[i].getX() && endpos_x <= this.vertices[i+1].getX()) {
                let startpos = this.vertices[i];
                ...
                let xDiff = endpos_x - this.vertices[i].getX();
                let perc = xDiff / (this.vertices[i+1].getX() - this.vertices[i].getX());
                lala2 = startpos.add(diff.scalarmult(perc));
            }
        }


        graphicsObject.stroke(230, 50, 0);
        graphicsObject.line(lala1.getX(), lala1.getY(), lala2.getX(), lala2.getY())

        if (this.id==2) {
            graphicsObject.fill(255);
            graphicsObject.noStroke();
            graphicsObject.circle(s.w*0.35, s.h*0.3, 30)
        }
